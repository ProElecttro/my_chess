...
function Game(props) {
    ...
    useEffect(() => {
        socket.on('connect', () => {
            ...
            socket.on('setPlayerOne', (socket)=>{
                setPlayer1(socket.id);
                console.log('player one is connected', socket.id, player1);
            })
    
            socket.on('setPlayerTwo', (socket)=>{
                setPlayer2(socket.id);
                console.log('player two is connected', socket.id, player2);
            })
            
            socket.on("newStateReceived", ({ positions, whoseTurn, roomCode }) => {
    ...
    const handleOnClick = (x, y) => {
        if(((whoseTurn === "white" && socket.id == player2) || (whoseTurn === "black" && socket.id == player1))){
            return;
        }
        ...
        if (pick[0] === -1 && pick[1] === -1) {
            ...
            socket.emit("statechange", { positions, whoseTurn, roomCode });
